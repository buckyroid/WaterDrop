...
class WaveBackground extends StatefulWidget {
  ...
  const WaveBackground({Key? key, required this.waterLevel}) : super(key: key);
...
class _WaveBackgroundState extends State<WaveBackground>
  ...
  Widget _buildAnimation(BuildContext context, Widget? widget, int waterLevel) {
    return SizedBox(
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, widget) =>
          _buildAnimation(context, widget, this.widget.waterLevel),
    );
  }
}
...
class WavePainter extends CustomPainter {
  ...
  void drawWave(Path path, int wave, size, waterLevel) {
    double waveWidth = size.width / waveSegments;
    double waveMinHeight = size.height / waterLevel;
    ...
    if (wave <= waveSegments) {
      drawWave(path, wave + 1, size, waterLevel);
    }
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    drawWave(path, 0, size, waterLevel);
